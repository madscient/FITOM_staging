...
# OUT (0-99) → OPM TL 変換テーブル (実機ルックアップ、非線形)
# 参考: https://nornand.hatenablog.com/entry/2021/08/22/172147
# (元記事はVolumeパラメータ用テーブルだが、OUTも同じTL=0(最大音量)からの
#  減衰加算値として機能するため同一テーブルを適用する)
OUT_TO_TL_TABLE = [
...
def out_to_tl(out):
    """OUT (0-99) → OPM TL (0-127) 実機ルックアップテーブル使用"""
    return OUT_TO_TL_TABLE[max(0, min(99, out))]
