...
## <a id="dsg-rhythm"></a>DSG(YM2163)内蔵リズム — CC#32=68
...
> DSGのリズム音は音程を変えるレジスタを持たないため、ノート番号に
> よらず常に同じ音程で鳴ります。またワンショット発音でキーオフに
> 相当する機構が無いため、ノートオフでは何も起きません。
>
> Hi-Hat OpenとHi-Hat Closeは実機で同一の発振器・同一の音量レジスタを
> 共有します。そのため、片方の音量を変えるともう片方にも同じ音量が
> 適用されます。
>
→ 実際のノート割り当ては[DSG Built-in set](drumkits.md#prog14-dsg-built-in-set)を参照

> DSGは5音しか持たないため、このキットで実際に鳴るのは
> バスドラム・スネア・ハイハット・コンガ/ボンゴ系の14ノートだけです。
> 対応する音が無いノートはプレースホルダ(無音)になっています。
