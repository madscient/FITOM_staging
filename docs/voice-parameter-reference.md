...
`ops[0-3]`は**アルゴリズム図のチェーン順**（`ops[0]`=M1, `ops[1]`=C1, `ops[2]`=M2,
`ops[3]`=C2。ALG=0なら`ops[0]→ops[1]→ops[2]→ops[3]`の直列）で、**実機YM2151の
レジスタスロット順(M1,M2,C1,C2 = アドレス+0/+8/+16/+24)とは異なる**。
`COPM::kMap={0,2,1,3}`がレジスタ書き込み時に並び替える。キャリア判定
(`kCarrierMask`)もチェーン順の添字で定義されている（ALG=4なら`ops[1]`と`ops[3]`が
キャリア）。OPN系（`COPN::kOpMap={0,8,4,12}`）も同じチェーン順である。
...
### レジスタイメージからの変換: `AR`/`DR`と`TL`のビット幅

`AR`/`DR`(および`SR`)と`TL`は、実機レジスタ値からの変換規則が異なる。

| フィールド | 実機レジスタ幅 | 格納する値 | 理由 |
|---|---|---|---|
| `AR`/`DR`/`SR` | 4bit (0-15) | 実機値`<< 1` (0-30) | 全チップ共通の5bit「上位ビット表現」で保持し、ドライバ側(`ar4()`)が`>> 1`して4bitへ切り出す |
| `TL` | 6bit (0-63) | 実機値そのまま (0-63) | 全チップ共通の減衰量空間(0.75dB/step)で保持し、ドライバ側(`effTLToReg()`)はレンジのクランプのみ行いスケーリングしない |

`TL`を`<< 1`して格納してはならない。ステップ幅が1.5dBに読み替わって減衰量が
2倍になり、実機レジスタ値で32以上のオペレータは6bitのレンジ(47.25dB)を
超えて飽和し消音する。OPL/OPL2/OPL3(4OPモード含む)/OPLLで共通の規則である。
...
| EGTビット=0(パーカッシブ)、RRレジスタ=r | `SR = r << 1`(4bit→5bit)、`RR=r`(そのまま、ビット幅変換不要) |

**重要(2026年7月20日訂正)**: FITOM_Xは**OPL/OPL2/OPL3もOPLLと同じく、
キーオン/キーオフのたびに実機EGTビット/RRレジスタを動的に書き換える**
(旧記述は「`updateVoice`一度きりの静的な書き込みで完結し、OPLLだけが
`updateVoice`+`updateKey`の2段階書き込み」としていたが誤り。OPL系も
実際には以下の動的な書き込みを行う):

- **キーオン時**: 常に実機EGTビット=0(パーカッシブ)にして、FITOMの
  `SR`の値(4bit変換)をRRレジスタに書く。
- **キーオフ時**: 常に実機EGTビット=1(サステイン)にして、FITOMの
  `RR`の値をそのままRRレジスタに書く。

したがって**`RR`は`SR`の値に関わらず常にキーオフ時に実機へ反映される**
（旧記述にあった「`SR>0`のとき`RR`は無視される」というのは誤り）。
`RR=0`のキャリアオペレータは、`SR`が何であっても**キーオフ時に実機へ
EGTビット=1・RRレジスタ=0が書き込まれ、事実上消音しなくなる**
(実機のRRレジスタ値0は最も遅いレートであり、実用時間内では減衰し
きらない)。変換時は**実機EGTビットの値に関わらず常に`RR=変換元RR
レジスタ値`を格納しなければならない**(`SR`のみ実機EGTビットに応じて
0または変換元RRレジスタ<<1で分岐)。

この変換規則(FITOMの`SR`/`RR`と、実機EGTビット/RRレジスタの対応、
およびキーオン/キーオフでの動的書き換え)は、OPL/OPL2/OPL3(4opモード
含む)/OPLL系で共通する。OPLL固有の実装上の注意点(`updateVoice`/
`updateKey`の役割分担)はOPLL専用セクション(下記)を参照。
...
OPLLも、上記OPL系セクションの通りキーオン/キーオフのたびに実機EGT
ビット(bit5、通称EGTビット、"SUS"表記のこともある)とRRレジスタを
動的に書き換える。OPLLの実装は`updateVoice`(発音チャンネル確保時に
1回)と`updateKey`(キーオン/キーオフのたび)という2つの関数に分かれて
おり、実際にRRレジスタへ最終的な値を書き込むのは`updateKey`である点が
実装上の特徴だが、変換規則自体はOPL系と共通(2026年7月に検証・訂正。
旧記述は「OPL系は`updateVoice`一度きりの静的書き込みで完結するが
OPLLだけが動的」としていたが、実際はOPL系も同じ動的書き込みを行うため
誤りだった。上記OPL系セクション参照)。
...
`RR`は`SR`の値に関わらず常にキーオフ時に実機へ反映されるため、
**実機EGTビットの値に関わらず常に`RR=変換元RRレジスタ値`を格納
しなければならない**。この規則を誤ってOPL系の(誤った)静的規則と
思い込んで変換したデータが実際に「キーオフで消音しない」バグを
引き起こした実例が`banks/OPLL/opll_presets.hwbank.json`、
`docs/CLAUDE.md` 3.23/3.24参照。
