...
**副次的に発見(今回は未対応)**: 上記の合成元との照合作業中、
`banks/OPL3/opl2_merge/`の7ファイルが合成元OPL2バンクと`AM`(599op)・
`WS`(894op)で食い違っていることが判明した。3.19で`vma_convert.py`の
`AM`/`WS`ビット位置を修正して`ma2_vma`を再変換した際、派生バンクである
`opl2_merge`側へ反映していなかったことによる(3.26のSR/RR・3.27のprog除外は
反映済みだが、`AM`/`WS`だけが漏れていた)。4節に記載。

## 4. 未解決・要確認事項
（各節末尾で「4節に記載」とした項目をここにまとめている。本セクション
見出しが過去のある時点で欠落していたため、2026年7月29日に補完した。）

- 3.51のTL修正(OPL/OPL2/OPL3/OPLL全41ファイル)は、実機/エミュレータで実際に
  鳴らした聴感確認が未実施(データ側の整合性検証・変換元からの再変換一致のみ
  済み)。全オペレータの音量バランスが変わる大きな変更であり、特に修正前は
  飽和して消音していた665opが鳴るようになるため、実際の発音確認が望ましい。
- `banks/OPL3/opl2_merge/`配下7ファイルの`AM`(599op)・`WS`(894op)が、合成元の
  `banks/OPL2/ma2_vma/`と食い違っている(3.51の副次的発見)。3.19で
  `vma_convert.py`のAM/WSビット位置を修正して`ma2_vma`を再変換した際、派生
  バンク側への反映が漏れたもの。`opl2_merge.py`での再合成はPDT等の後付け
  フィールドが失われるため、3.26/3.27と同様にprog単位・op単位で該当2
  フィールドのみをin-place反映するのが安全。
