...
- `banks/OPL2/`・`banks/OPL3/`・`banks/OPLL/`配下の全`*.hwbank.json`
  ...
  `banks/OPLL/opll_presets.hwbank.json`はこのリポジトリに変換スクリプトが
  残っていない(過去セッションでの変換と推測)が、同じ「4bit/4bit/6bitの
  生値がそのまま格納されている」分布だったため、同じ規則で修正可能と判断
  し変換元への遡り無しで直接修正した。
...
- OPN/OPM/OPZ/PSG系（`FV_AR_OPN`は無シフト、`FV_AR_OPM`は`>>2`)は
  対象外（今回report範囲外、必要なら別途確認要）。
