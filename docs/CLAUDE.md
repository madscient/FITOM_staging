...
- **【保留】PCED `LFOS=3`(Vibrato)の音色のソフトLFO変換**(2026年8月10日、
  ...
  - 変換を入れる場合に決める必要がある点: (1) PCEDが無いためLFOSの値をどう
    ...
    線形写像するのは誤り。
- 3.46でDX/TX81Z/FB-01由来swbankの全720パッチに`VTL=80`を与えたが、これは
  4オペレータ全てに一律で適用している。3.21の設計原則(TLが音量として作用するのは
  キャリアオペレータのみで、モジュレータのTLは音色の明るさに作用する)に従うなら、
  各パッチ自身のALGから求めたキャリアopのみに限定するのが筋。3.21がこの6バンクを
  対象外にした根拠は3.46で成立しないと判明しているため、キャリア限定へ変更するか
  要判断(これらは1パッチ1専用`sw_prog`なので、`performance_presets`のprog24-31の
  ようなパターン別バリアントは不要で、各パッチに直接設定できる)。
