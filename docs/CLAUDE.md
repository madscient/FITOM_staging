...
| 127 | 0x7F | SILENCE（無音バンク。hwBank/hwProgを見ず「解決成功・ただし発音しない」を返す唯一の経路。プレースホルダ用。3.57参照） |
...
**【3.51で部分的に訂正】**: 本節のうち**`TL`に関する記述は現在では誤り**。
本節の2日後(2026年7月20日、FITOM_X側コミット`8aa9201`)にランタイム側の
`tl6()`(`>>1`)が`effTLToReg()`(dB変換、スケーリングなし)へ置き換わったため、
`TL`は`<<1`せず実機レジスタ値(0-63)をそのまま格納するのが正しい。
`AR`/`DR`/`SR`の`<<1`(本節の対応)は`ar4()`が現存するため引き続き正しい。
詳細は3.51参照。
...
### 3.51 OPL/OPLL系TLの`<<1`を撤回（2026年8月10日、ユーザー指摘）
ユーザーから「OPL系パッチのTLは0-63だが、変換時に0-127へのストレッチが
されていないか（しないのが正しい）」との確認依頼を受け調査した結果、
**3.17で導入した`TL <<1`が現在のランタイム仕様では誤り**であることが
判明したため撤回した。`AR`/`DR`/`SR`の`<<1`は引き続き正しく、対象外。

**現在のランタイム仕様**: `TL`は**チップを問わず0.75dB/stepの減衰量空間**で
保持する共通仕様で、OPL系はレンジが6bit(47.25dB)に狭いだけである。
`COPL`/`COPL3`/`COPLRhythm`/`COPLL`はいずれも
`effTLToReg()`(`COPLL`は`modTLToReg()`) = `linear2dB(effTL, RANGE48DB,
STEP075DB, 6)`でレジスタへ書くが、`RANGE48DB=1`・`bw=6`のため
`linear2dB()`末尾のシフト量は`7-1-6=0`で、**クランプのみ行いスケーリングは
一切しない**(`OPL_new.cpp`のコメント「ここはクランプであってスケーリング
ではない — TL>>1 のような縮小を掛けるとステップ幅が1.5dBに読み替わり
減衰量が半分になる」)。実際に`kGM2dB`テーブルを使って
`hwOp.TL → calcLinearLevel → linear2dB`の往復を数値シミュレーションした
ところ、TL 0-62はレジスタへ1:1で透過し、**63以上は62(実質消音)に飽和**する。

**経緯**: 3.17(2026年7月18日)の時点ではランタイム側が`tl6() { return v >> 1; }`
で読んでいたため`<<1`が正しかったが、その2日後のFITOM_X側コミット`8aa9201`
(2026年7月20日「TLレジスタ書き込み時にラウドネス→減衰量dB変換が欠落」の修正)
で`tl6()`が`effTLToReg()`に置き換わってスケーリングが消滅し、さらに`16d5081`
(2026年8月10日)で`tl6()`自体が削除された。本リポジトリ側はこの変更に追従
できておらず、約3週間ずれたままだった。`VoiceData.h`の
`FV_TL_OPL(v,o) ((v).hwOp[o].TL >> 1)`マクロは残っているが**どこからも
呼ばれていない死んだマクロ**である(3.17でOPM側の`FV_TL_OPM`について判明した
のと同じ状況)。

**実害**: 全オペレータで減衰量がdBで2倍になっていた(例: レジスタ値20相当の
オペレータが40 = 15dB余分に減衰)。さらにレジスタ値32以上だったオペレータは
格納値64以上となり62へ飽和し**実質消音**していた(該当665op / 全8086op、
8.2%、38ファイル)。

**対応**:
- `alsa_convert.py`・`vma_convert.py`・`opll_convert.py`の`TL`から`<< 1`を
  削除(`AR`/`DR`/`SR`は変更なし)。各スクリプト冒頭のフォーマット解説にも
  「TLは`<<1`してはならない」旨とその理由を明記。
- `banks/OPL2/`・`banks/OPL3/`・`banks/OPLL/`配下の全hwbank(41ファイル、
  8086op)の`TL`を`>>1`。3.17の`<<1`により**全件が偶数**だったため完全に
  可逆で、実機レジスタ値(0-63)が正確に復元される。
- `docs/voice-parameter-reference.md`のOPL系節に「`AR`/`DR`と`TL`のビット幅」
  の対比表を追加。

**検証**:
- 変換元(`E:\マイドライブ\FITOM\material\fmvoice\`の`vma/*.vma`27件・
  `std.sb`/`std.o3`/`drums.sb`/`drums.o3`、およびOPLL用レジスタダンプ
  3ファイル)から修正後スクリプトで再変換し、全31+1ファイル・
  77168フィールド(うちTL 5936)が**不一致0件**であることを確認。
- git HEADとの差分が`"TL"`の行のみ(他フィールド・メタ情報・改行コードとも
  差分0)であることを確認。
- `banks/OPL3/opl2_merge/`配下7ファイル(`opl2_merge.py`は値を再計算せず
  入力を引き継ぐ設計、3.10参照)は、合成元OPL2バンクとのprog単位・op単位の
  照合で`TL`が完全一致することを確認(再変換不要)。
- `banks/OPL2/msx_audio/`2ファイル(変換元スクリプトなし、3.17/3.25参照)も
  同じ`>>1`で対応。最大TLが126→63になり6bitレンジに収まることを確認。
- `hwbank.schema.json`で42ファイル全てVALID。
- 実機/エミュレータでの聴感確認もユーザーにより完了している。

**副次的に発見**: 上記の合成元との照合作業中、`banks/OPL3/opl2_merge/`の
7ファイルが合成元OPL2バンクと`AM`(599op)・`WS`(894op)で食い違っていることが
判明した(3.52で修正)。

### 3.52 opl2_merge派生バンクへ`AM`/`WS`のビット位置修正を反映（2026年8月10日）
3.51の副次的発見(`banks/OPL3/opl2_merge/`の7ファイルが合成元OPL2バンクと
`AM`/`WS`で食い違う)に対応した。3.19で`vma_convert.py`の`AM`/`WS`ビット位置を
修正して`banks/OPL2/ma2_vma/`を再変換した際、派生バンクである`opl2_merge`側へ
反映していなかったことによる(3.26のSR/RR・3.27のprog除外・3.51のTLは反映済み
だが、`AM`/`WS`だけが漏れていた)。

**原因の裏付け**: 全1848 opについて、`opl2_merge`側の`WS`下位2bitが
旧ビット位置での復号結果`(新AM << 1) | (新WS >> 2)`と一致することを確認した
(旧: `AM`=bit5・`WS`=bits4-2、新: `AM`=bit3・`WS`=bits2-0。説明不能な箇所は
0件)。すなわち差分は3.19の反映漏れのみで、他の要因は無い。

**対応**: `opl2_merge.py`での再合成は`PDT`等の後付けフィールドが失われるため、
3.26/3.27と同様に合成元(`source`フィールドが指すバンク)からprog単位・op単位で
`AM`/`WS`のみをin-place反映した(`ops[0/1]`←Bank-Aの`ops[0/1]`、`ops[2/3]`←
Bank-Bの`ops[0/1]`)。

**検証**: git HEADとの差分が`AM`/`WS`のみ(パース結果の全数比較で他フィールド・
メタ情報とも差分0)、合成元との全24024フィールド照合で不一致0件、
...
- BD(0): 35, 36 / SDN(2): 37, 38, 39, 40 / HHD(4): 42, 44 / HHO(3): 46 /
  HC(1): 60, 61, 62, 63, 64
- プレースホルダは`patch_prog=5`(パート数5の範囲外)としていたが、
  **3.57で`voice_patch_type=127`方式へ差し替え済み**(範囲外検出による
  無音化はノートを叩くたびに警告ログが出るため)。楽器名は末尾に
  `(placeholder)`を付けて区別している。後から音を割り当てる際は
  `voice_patch_type=112`・`patch_bank=68`に戻して`patch_prog`を0-4にする。
...
### 3.57 全ドラムキットへGM2ノートの無音プレースホルダを敷き詰め（2026年8月16日、ユーザー指示）
`banks/drums/*.drumkit.json`(`type: "routed"`)全21ファイルについて、
GM2が定義するノート27-87のうち`notes[]`に無かったものを、GM2ノート名
+`(placeholder)`の**無音エントリ**として追加した。これで全routedキットが
一律61ノート(ma2系のみ拡張ノート88-105を含め79ノート)を持つ。
`opl4awm.drumkit.json`は`type: "direct"`で`note_min=27`/`note_max=87`の
ため対象外(既に全域をカバー済み)。

**無音の実現方法は`voice_patch_type=127`**(`VOICE_PATCH_SILENCE`=0x7F。
`patch_bank`/`patch_prog`はdon't careで0を入れてある)。
`PatchManager::resolveTriple()`の入口で`silent=true`だけを立てて返す
専用経路で、チップ種別もhwBank/hwProgも一切参照しない。**「解決成功・
ただし発音しない」**(`layerCount==0`の有効な`ResolvedPatch`)という第3の
結果を返すため、デバイス構成に依存せず必ず無音になり、警告ログも出ない。
3.55のDSGキットで使っていた「存在しないビルトインパート番号
(`patch_prog=5`)を指す」方式は解決**失敗**であり、叩くたびに
`resolveBuiltinRhythm`の範囲外警告と`CRhythmCh: Patch not found`が
出るため不十分だった。47件すべてをこの方式へ差し替えた。

**3.40のSF2バンク表示(CC#0=127)との関係**: 3.40の「127はFITOM_X側で
特に意味を持たない値」という前提は誤りで、実際には`VOICE_PATCH_SILENCE`
として本体側で定義済みだった。ただしSF2直行パスは`sf2_channel_windows`
(MPU×ch単位の窓)で切り分けられ、**CC#0による切替は設計上不採用**
(窓に入ったchではCC#0は読み捨てられる)のため、実動作上の衝突は無い。
影響はインストゥルメントリストの表示規約のみ — 窓に入っていないchで
CC#0=127を選ぶと、SF2ではなく無音になる。4節に記載。

| ファイル | 追加 | ファイル | 追加 |
|---|---|---|---|
| `opna_builtin` | 43 | `rx5_extra_opnb` | 38 |
| `rx5_opnb` | 41 | `pss560_opnb` | 35 |
| `pss590_opnb`/`_power`/`_electronic` | 各42 | `rx11_21l_opnb`/`_ssgs` | 各32 |
| `opl_builtin_rhythm`/`opll_rhythm` | 各40 | `pss680_opnb`/`_brush`/`_ssgs` | 各26 |
| `alsa_preset_2op`/`_4op` | 各14 | `dsg_builtin_rhythm` | 0(47件を方式変更) |

`gm2_standard`/`opm_standard`/ma2系7ファイルは既に27-87を網羅しており
変更なし。既存ノートの`patch_bank`/`patch_prog`/`pan`/`gate_time`等には
一切手を触れていない(追記と、DSGプレースホルダの参照先差し替えのみ)。

**あわせてスキーマを再同期**(5.5節): `drumkit.schema.json`・
`patchbank.schema.json`の2ファイルは、FITOM_X側で`voice_patch_type`の
説明に0x7Fの記述が追加されており、staging側が古かった。
本体からverbatimコピーして再び全11ファイル一致状態に戻した
(内容は説明文のみの変更で、制約[`minimum`/`maximum`等]に変化なし)。

**検証**: 全27ファイルが`drumkit.schema.json`でVALID、ノート番号重複0件、
GM2ノート27-87の欠落0件。インストゥルメントリストを再生成し、
...
`Y8960emuEngine`、チップ名`OPLLEX`/`OPL2EX`)。
...
配線した。エミュレーターは`../Y8960emu`(`Y8960emuEngine`、FmEngineApi準拠)。
...
公開しているのはこのため。Y8960emuは当初`"Y8960_OPLLX"`/`"Y8960_OPL2"`
のみを受け付けていたが、この規約に合わせて`"OPLLEX"`/`"OPL2EX"`へ改称
してもらった(同時にDLL名も`Y8960EngineApi`→`Y8960emuEngine`へ改称。
`engines[].dll`・`devices[].engine`の両方がこのファイル名を指す)。
...
|---|---|---|
| `emu_opll` | `fmemuif_opll5.profile.json` | ×1(pan 0)。Y8960emuEngineを2つ目の`engines[]`エントリとして追加 |
...
`bin/fitom_cli.exe`で3プロファイルとも実起動し、`HWPort opened:
{"chip":"OPLLEX","engine":"engines/Y8960emuEngine",...}`が期待数だけ出て
`Failed to create HWPort`が0件であること、`fmall`では8スロット全てが開き
`OPZ`/`OPNBB`(FM/SSG/ADPCM-A/ADPCM-B)/`OPL3-4OP←OPL4-FM-4OP`/
`OPL3-2OP←OPL4-FM-2OP`のペアが`[chip-level L/R]`で、`emu_opll_stereo`の
OPLL系5ペアが`[plugin-routed L/R]`で束ねられることを確認した
(OPL4のAWM部は設計どおり独立24chのまま)。**ただしOPLLEXデバイス自体は
`bin/`のバイナリが古く発音まで到達していない**(下記)。

**`bin/`のFITOM_X本体がOPLLEX未対応**: 現在配置されている
`bin/fitom_cli.exe`は2026年8月16日ビルドで、`resolveChipDeviceId()`へ
`{"OPLLEX", DEVICE_OPLLEX}`が入ったFITOM_X側コミット`746431e`(8月20日)より
古い。このため`resolveChipDeviceId: unknown chip 'OPLLEX'`→
`Device[n] 'OPLLEX': deviceType unknown, skipping`となり、ポートは開くが
...
- **`bin/`のFITOM_X本体を再ビルドする必要がある**(3.59): 配置済みの
  ...
  生成されない**。FITOM_Xを`746431e`以降で再ビルドし`setup.ps1`を
  実行し直すこと。その後の発音確認(聴感)も未実施。
...
- **CC#0=127の二重の意味**(3.57): FITOM_X本体では`VOICE_PATCH_SILENCE`
  ...
  ため実害は無いが、リスト上で「SF2バンク」に見えるものを窓外のchで選ぶと
  無音になる。表示規約を別のCC#0値へ移すか、リスト側に注記を足すか要判断。
