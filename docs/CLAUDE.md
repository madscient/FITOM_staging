...
`hwbank.schema.json`で7ファイルVALID。

## 4. 未解決・要確認事項
（各節末尾で「4節に記載」とした項目をここにまとめている。本セクション
見出しが過去のある時点で欠落していたため、2026年7月29日に補完した。）

- 3.51のTL修正(OPL/OPL2/OPL3/OPLL全41ファイル)・3.52の`AM`/`WS`反映は、実機/
  エミュレータで実際に鳴らした聴感確認が未実施(データ側の整合性検証・変換元
  からの再変換一致のみ済み)。特にTLは全オペレータの音量バランスが変わる大きな
  変更で、修正前は飽和して消音していた665opが鳴るようになるため、実際の発音
  確認が望ましい。
